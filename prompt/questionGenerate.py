def get_question_generation_prompt(text: str, max_qs: int = 3) -> dict:
    system_content = (
...
## 要求：
- 文本中可能包含图片占位符 [IMAGE_1], [IMAGE_2] 等。这些占位符表示文档中的图片。
- 可以针对图片内容生成问题，例如“请描述图片 [IMAGE_1] 中的场景”。
- 问题必须基于文本内容和图片内容直接生成
- 问题应具有明确答案指向性,但不要给出答案
- 问题类型可以多样：事实型、推理型、填空型
- 禁止生成假设性、重复或相似问题，禁止虚构不存在的图片内容
- 优先覆盖文本中的关键信息点
- 输出 JSON 格式如下,严格遵守：
["问题1","问题2","..."]
